<p align="center">
  <a href="https://www.tusyndon.com/">
    <img src="assets/tusyndon-solid-designer-hero.png" width="100%" alt="Tusyndon Industrial Software — parametrische Engineering-Plattform SolidDesigner" />
  </a>
</p>

<p align="center">
  <a href="https://www.tusyndon.com/"><img alt="Tusyndon Unternehmenswebsite" src="https://img.shields.io/badge/TUSYNDON-COMPANY-082B68?style=for-the-badge&logo=googlechrome&logoColor=white" /></a>
  <a href="https://www.tusyndon.com/products"><img alt="Tusyndon Produktportfolio" src="https://img.shields.io/badge/ENGINEERING-PRODUCTS-0B63CE?style=for-the-badge" /></a>
  <a href="https://github.com/Ludwigstrasse/SolidDesigner"><img alt="SolidDesigner Repository" src="https://img.shields.io/badge/OPEN_PLATFORM-SolidDesigner-009E95?style=for-the-badge&logo=github&logoColor=white" /></a>
  <a href="mailto:sales@tusyndon.com"><img alt="Tusyndon kontaktieren" src="https://img.shields.io/badge/CONTACT-BOOK_A_DEMO-13A99B?style=for-the-badge&logo=minutemailer&logoColor=white" /></a>
</p>

<p align="center">
  <a href="README.md">English</a> · <a href="README.zh.md">简体中文</a> · <a href="README.ja.md">日本語</a> ·
  <a href="README.fr.md">Français</a> · <a href="README.de.md">Deutsch</a> · <a href="README.es.md">Español</a> ·
  <a href="README.ru.md">Русский</a> · <a href="README.hi.md">हिन्दी</a> · <a href="README.th.md">ไทย</a> ·
  <a href="README.vi.md">Tiếng Việt</a> · <a href="README.pt.md">Português</a> · <a href="README.ms.md">Bahasa Melayu</a> ·
  <a href="README.id.md">Bahasa Indonesia</a> · <a href="README.ko.md">한국어</a> · <a href="README.kk.md">Қазақша</a> ·
  <a href="README.mn.md">Монгол</a> · <a href="README.uk.md">Українська</a> · <a href="README.cs.md">Čeština</a> ·
  <a href="README.eu.md">Euskara</a> · <a href="README.ta.md">தமிழ்</a> · <a href="README.bn.md">বাংলা</a>
</p>

## Engineering-Software für reale Produktentwicklung

**Tusyndon Industrial Software** entwickelt spezialisierte Anwendungen für mechanische Konstruktion, Fertigungsvorbereitung, technische Auslieferung und BIM-Datenabläufe. **SolidDesigner** ist die offene Engineering-Leitplattform: eine Desktop- und Workbench-orientierte Grundlage für historienbasierte parametrische CAD-Konstruktion und simulationsgetriebene Entwicklung.

<table>
  <tr>
    <td width="50%" valign="top">
      <h3>Kommerzielle Engineering-Produkte</h3>
      <p>Spezialisierte Werkzeuge automatisieren Zeichnungsausgabe, Modellprüfung, Baugruppenarchitektur, Blechvorbereitung, Druckbehälterauslegung, IFC-Abläufe und strukturierte technische Innovation.</p>
      <p><a href="https://www.tusyndon.com/products"><strong>Tusyndon Produkte entdecken →</strong></a></p>
    </td>
    <td width="50%" valign="top">
      <h3>Offene Engineering-Plattform</h3>
      <p>SolidDesigner verbindet parametrische Konstruktionsabsicht, OCCT-B-Rep-Geometrie, Dokumenttransaktionen, dauerhafte technische Identität und professionelle Desktop-Workbenches.</p>
      <p><a href="https://github.com/Ludwigstrasse/SolidDesigner"><strong>SolidDesigner entdecken →</strong></a></p>
    </td>
  </tr>
</table>

<p align="center">
  <a href="https://www.tusyndon.com/products">
    <img src="assets/tusyndon-product-system.svg" width="100%" alt="Tusyndon Produktlandschaft mit SolidDesigner und vier kommerziellen Produktfamilien" />
  </a>
</p>

## Tusyndon Produktfamilien

<table>
  <tr>
    <td width="50%" valign="top">
      <h3>Zeichnungsautomatisierung</h3>
      <p><strong>Tusyndon Drawing Nester</strong><br />Automatische Creo-Zeichnungsanordnung, Optimierung der Papiernutzung und kontrollierte Druckwarteschlangen.</p>
      <p><strong>Tusyndon Drawing Publisher</strong><br />Regelbasierte Stapelveröffentlichung nach PDF und DXF mit strukturierten Auslieferungspaketen.</p>
    </td>
    <td width="50%" valign="top">
      <h3>Modellintelligenz</h3>
      <p><strong>Tusyndon PMI Reviewer</strong><br />Vollständigkeitsprüfung von PMI, MBD, GD&amp;T und Bezügen mit direkt bearbeitbaren Problemberichten.</p>
      <p><strong>Tusyndon Assembly Architect</strong><br />Baugruppenstrukturen, Systemschnittstellen und Architekturansichten für komplexe Anlagen.</p>
    </td>
  </tr>
  <tr>
    <td width="50%" valign="top">
      <h3>Fertigungsengineering</h3>
      <p><strong>Tusyndon Sheet Metal Nester</strong><br />DXF-Verschachtelung, Optimierung der Materialausnutzung und Vorbereitung von Schnittlisten.</p>
      <p><strong>Tusyndon Pressure Vessel Designer</strong><br />Parametrische Druckbehälterkonstruktion, technische Berechnung und Simulationsvorbereitung.</p>
    </td>
    <td width="50%" valign="top">
      <h3>BIM und technische Innovation</h3>
      <p><strong>Tusyndon IFC Editor · Compare · Takeoff</strong><br />IFC-Anzeige, Eigenschaftsbearbeitung, Versionsvergleich und Mengenermittlung.</p>
      <p><strong>Tusyndon TRIZ Studio</strong><br />Strukturierte Widerspruchsanalyse und Methoden für technische Innovation.</p>
    </td>
  </tr>
</table>

## SolidDesigner

<table>
  <tr>
    <td width="42%" valign="top">
      <h3>Parametrische Engineering-Plattform</h3>
      <p>SolidDesigner ist eine moderne C++17/20-Desktopanwendung auf Basis des wiederverwendbaren <strong>Alice</strong>-Fundaments und des <strong>OpenCascade</strong>-Geometriekerns.</p>
      <p>Das Projekt befindet sich in <strong>aktiver Pre-Alpha-Entwicklung</strong>. Der aktuelle Quellcode etabliert Anwendungshülle, technische Datengrundlage, frühe Feature- und Zwangsbedingungssysteme, transaktionsbewusste Befehle und eine erweiterbare Workbench-Struktur.</p>
      <p><a href="https://github.com/Ludwigstrasse/SolidDesigner#build--run"><strong>Mit Visual Studio 2022 erstellen →</strong></a></p>
    </td>
    <td width="58%" valign="top" align="center">
      <img src="assets/solid-designer-workbench.png" width="100%" alt="Aktuelle SolidDesigner Home Workbench" />
      <br />
      <sub>Aktuelle SolidDesigner / Breptera Home Workbench · aktive Pre-Alpha-Entwicklung</sub>
    </td>
  </tr>
</table>

<p align="center">
  <img alt="C++ 17 und 20" src="https://img.shields.io/badge/C%2B%2B-17%20%7C%2020-00599C?logo=cplusplus&logoColor=white" />
  <img alt="Visual Studio 2022" src="https://img.shields.io/badge/Visual_Studio-2022-5C2D91?logo=visualstudio&logoColor=white" />
  <img alt="Qt 5.15" src="https://img.shields.io/badge/Qt-5.15-41CD52?logo=qt&logoColor=white" />
  <img alt="OpenCascade" src="https://img.shields.io/badge/Geometry-OpenCascade-0B63CE" />
  <img alt="Windows und Linux" src="https://img.shields.io/badge/Platform-Windows%20%7C%20Linux-455A64?logo=windows11&logoColor=white" />
  <a href="https://github.com/Ludwigstrasse/SolidDesigner/blob/main/LICENSE"><img alt="AGPL 3.0" src="https://img.shields.io/badge/License-AGPL--3.0-3DA639" /></a>
</p>

## Engineering-Grundlagen

<table>
  <tr>
    <td width="50%" valign="top">
      <h3>Parametrisches Modell</h3>
      <p>Skizzeneinschränkungen, benannte Maße, Feature-Abhängigkeiten, deterministische Regenerierung und Änderungs- oder Neudefinitionsabläufe erhalten die Konstruktionsabsicht.</p>
    </td>
    <td width="50%" valign="top">
      <h3>Geometrie und Topologie</h3>
      <p>OCCT-gestützte B-Rep-Modellierung bietet Volumen-, Flächen-, Topologie- und Visualisierungsfunktionen für professionelle CAD-Abläufe.</p>
    </td>
  </tr>
  <tr>
    <td width="50%" valign="top">
      <h3>Dokumentintegrität</h3>
      <p>Dokumenttransaktionen, ObjectId-basierte Identität, persistente Referenzen sowie befehlsbezogenes Rückgängigmachen und Wiederherstellen sichern die Modellkontinuität.</p>
    </td>
    <td width="50%" valign="top">
      <h3>Professioneller Desktop</h3>
      <p>Menübandbefehle, andockbare Bereiche, MDI-Ansichten, Auswahl, Interaktion und modulare Workbenches unterstützen die langfristige Produktentwicklung.</p>
    </td>
  </tr>
</table>

## Mit Tusyndon zusammenarbeiten

<p align="center">
  <strong>Bringen Sie einen realen Engineering-Ablauf, ein repräsentatives Modell, einen Zeichnungssatz oder ein IFC-Projekt mit.</strong><br />
  Wir bewerten die Produkteignung anhand der tatsächlichen Eingangsdaten, des Prozesses und der benötigten Ergebnisse.
</p>

<p align="center">
  <a href="https://www.tusyndon.com/solutions"><strong>Engineering-Lösungen</strong></a>
  &nbsp; · &nbsp;
  <a href="https://www.tusyndon.com/industries"><strong>Branchen</strong></a>
  &nbsp; · &nbsp;
  <a href="mailto:sales@tusyndon.com"><strong>Produktdemo buchen</strong></a>
  &nbsp; · &nbsp;
  <a href="https://github.com/Ludwigstrasse/SolidDesigner/discussions"><strong>Technische Diskussionen</strong></a>
</p>

---

<p align="center">
  <strong>Tusyndon Industrial Software</strong><br />
  <sub>Mechanische Konstruktion · Fertigungsvorbereitung · technische Auslieferung · BIM-Daten</sub><br /><br />
  <a href="https://www.tusyndon.com/">Unternehmen</a> ·
  <a href="https://www.tusyndon.com/products">Produkte</a> ·
  <a href="https://github.com/Ludwigstrasse/SolidDesigner">SolidDesigner</a> ·
  <a href="https://github.com/hananiahhsu/SolidDesignerWiki">Öffentliches Wiki</a> ·
  <a href="mailto:sales@tusyndon.com">sales@tusyndon.com</a>
</p>

<details>
  <summary><strong>Engineering-Aktivität auf GitHub</strong></summary>
  <br />
  <p align="center">
    <img src="assets/stats/metrics.svg" width="100%" alt="Kennzahlen zur Engineering-Aktivität auf GitHub" />
  </p>
</details>
