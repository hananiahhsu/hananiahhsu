<p align="center">
  <a href="https://www.tusyndon.com/">
    <img src="assets/tusyndon-solid-designer-hero.png" width="100%" alt="Tusyndon Industrial Software — platform kejuruteraan parametrik SolidDesigner" />
  </a>
</p>

<p align="center">
  <a href="https://www.tusyndon.com/"><img alt="Laman syarikat Tusyndon" src="https://img.shields.io/badge/TUSYNDON-COMPANY-082B68?style=for-the-badge&logo=googlechrome&logoColor=white" /></a>
  <a href="https://www.tusyndon.com/products"><img alt="Portfolio produk Tusyndon" src="https://img.shields.io/badge/ENGINEERING-PRODUCTS-0B63CE?style=for-the-badge" /></a>
  <a href="https://github.com/Ludwigstrasse/SolidDesigner"><img alt="Repositori SolidDesigner" src="https://img.shields.io/badge/OPEN_PLATFORM-SolidDesigner-009E95?style=for-the-badge&logo=github&logoColor=white" /></a>
  <a href="mailto:sales@tusyndon.com"><img alt="Hubungi Tusyndon" src="https://img.shields.io/badge/CONTACT-BOOK_A_DEMO-13A99B?style=for-the-badge&logo=minutemailer&logoColor=white" /></a>
</p>

<p align="center">
  <a href="README.md">English</a> · <a href="README.zh.md">简体中文</a> · <a href="README.ja.md">日本語</a> ·
  <a href="README.fr.md">Français</a> · <a href="README.de.md">Deutsch</a> · <a href="README.es.md">Español</a> ·
  <a href="README.ru.md">Русский</a> · <a href="README.hi.md">हिन्दी</a> · <a href="README.th.md">ไทย</a> ·
  <a href="README.vi.md">Tiếng Việt</a> · <a href="README.pt.md">Português</a> · <a href="README.ms.md">Bahasa Melayu</a> ·
  <a href="README.id.md">Bahasa Indonesia</a> · <a href="README.ko.md">한국어</a> · <a href="README.kk.md">Қазақша</a> ·
  <a href="README.mn.md">Монгол</a> · <a href="README.uk.md">Українська</a> · <a href="README.cs.md">Čeština</a> ·
  <a href="README.eu.md">Euskara</a> · <a href="README.ta.md">தமிழ்</a> · <a href="README.bn.md">বাংলা</a>
</p>

## Perisian kejuruteraan untuk pembangunan produk sebenar

**Tusyndon Industrial Software** membangunkan aplikasi khusus untuk reka bentuk mekanikal, persediaan pembuatan, penyerahan kejuruteraan dan aliran data BIM. **SolidDesigner** ialah platform kejuruteraan terbuka utama: asas desktop berorientasikan meja kerja untuk CAD parametrik berasaskan sejarah dan reka bentuk berpandukan simulasi.

<table>
  <tr>
    <td width="50%" valign="top">
      <h3>Produk kejuruteraan komersial</h3>
      <p>Alat khusus mengautomasikan penyerahan lukisan, semakan model, seni bina pemasangan, persediaan kepingan logam, kejuruteraan bekas tekanan, aliran IFC dan inovasi berstruktur.</p>
      <p><a href="https://www.tusyndon.com/products"><strong>Terokai produk Tusyndon →</strong></a></p>
    </td>
    <td width="50%" valign="top">
      <h3>Platform kejuruteraan terbuka</h3>
      <p>SolidDesigner menggabungkan niat reka bentuk parametrik, geometri B-Rep OCCT, transaksi dokumen, identiti kejuruteraan kekal dan meja kerja desktop profesional.</p>
      <p><a href="https://github.com/Ludwigstrasse/SolidDesigner"><strong>Terokai SolidDesigner →</strong></a></p>
    </td>
  </tr>
</table>

<p align="center">
  <a href="https://www.tusyndon.com/products">
    <img src="assets/tusyndon-product-system.svg" width="100%" alt="Sistem produk perisian kejuruteraan Tusyndon dengan SolidDesigner dan empat keluarga komersial" />
  </a>
</p>

## Keluarga produk Tusyndon

<table>
  <tr>
    <td width="50%" valign="top">
      <h3>Automasi lukisan</h3>
      <p><strong>Tusyndon Drawing Nester</strong><br />Susun atur automatik lukisan Creo, pengoptimuman penggunaan kertas dan barisan cetak terkawal.</p>
      <p><strong>Tusyndon Drawing Publisher</strong><br />Penerbitan kelompok PDF dan DXF berasaskan peraturan dengan pakej penyerahan berstruktur.</p>
    </td>
    <td width="50%" valign="top">
      <h3>Kecerdasan model</h3>
      <p><strong>Tusyndon PMI Reviewer</strong><br />Semakan kelengkapan PMI, MBD, GD&amp;T dan datum dengan laporan isu yang boleh diambil tindakan.</p>
      <p><strong>Tusyndon Assembly Architect</strong><br />Struktur pemasangan, antara muka sistem dan pandangan seni bina untuk peralatan kompleks.</p>
    </td>
  </tr>
  <tr>
    <td width="50%" valign="top">
      <h3>Kejuruteraan pembuatan</h3>
      <p><strong>Tusyndon Sheet Metal Nester</strong><br />Penyusunan DXF, pengoptimuman penggunaan bahan dan penyediaan senarai potongan.</p>
      <p><strong>Tusyndon Pressure Vessel Designer</strong><br />Reka bentuk parametrik bekas tekanan, pengiraan kejuruteraan dan persediaan simulasi.</p>
    </td>
    <td width="50%" valign="top">
      <h3>BIM dan inovasi kejuruteraan</h3>
      <p><strong>Tusyndon IFC Editor · Compare · Takeoff</strong><br />Paparan IFC, penyuntingan sifat, perbandingan versi dan pengiraan kuantiti.</p>
      <p><strong>Tusyndon TRIZ Studio</strong><br />Analisis percanggahan berstruktur dan kaedah inovasi kejuruteraan.</p>
    </td>
  </tr>
</table>

## SolidDesigner

<table>
  <tr>
    <td width="42%" valign="top">
      <h3>Platform kejuruteraan parametrik</h3>
      <p>SolidDesigner ialah aplikasi desktop C++17/20 moden yang dibina atas asas <strong>Alice</strong> boleh guna semula dan kernel geometri <strong>OpenCascade</strong>.</p>
      <p>Projek kini dalam <strong>pembangunan pra-alfa aktif</strong>. Kod semasa membentuk kerangka aplikasi, asas data kejuruteraan, sistem ciri dan kekangan awal, arahan yang menyokong transaksi serta struktur meja kerja boleh dikembangkan.</p>
      <p><a href="https://github.com/Ludwigstrasse/SolidDesigner#build--run"><strong>Bina dengan Visual Studio 2022 →</strong></a></p>
    </td>
    <td width="58%" valign="top" align="center">
      <img src="assets/solid-designer-workbench.png" width="100%" alt="SolidDesigner Home Workbench semasa" />
      <br /><sub>SolidDesigner / Breptera Home Workbench semasa · pra-alfa aktif</sub>
    </td>
  </tr>
</table>

<p align="center">
  <img alt="C++ 17 dan 20" src="https://img.shields.io/badge/C%2B%2B-17%20%7C%2020-00599C?logo=cplusplus&logoColor=white" />
  <img alt="Visual Studio 2022" src="https://img.shields.io/badge/Visual_Studio-2022-5C2D91?logo=visualstudio&logoColor=white" />
  <img alt="Qt 5.15" src="https://img.shields.io/badge/Qt-5.15-41CD52?logo=qt&logoColor=white" />
  <img alt="OpenCascade" src="https://img.shields.io/badge/Geometry-OpenCascade-0B63CE" />
  <img alt="Windows dan Linux" src="https://img.shields.io/badge/Platform-Windows%20%7C%20Linux-455A64?logo=windows11&logoColor=white" />
  <a href="https://github.com/Ludwigstrasse/SolidDesigner/blob/main/LICENSE"><img alt="AGPL 3.0" src="https://img.shields.io/badge/License-AGPL--3.0-3DA639" /></a>
</p>

## Asas kejuruteraan

<table>
  <tr>
    <td width="50%" valign="top"><h3>Model parametrik</h3><p>Kekangan lakaran, dimensi bernama, kebergantungan ciri, penjanaan semula deterministik dan aliran sunting atau takrif semula mengekalkan niat reka bentuk.</p></td>
    <td width="50%" valign="top"><h3>Geometri dan topologi</h3><p>Pemodelan B-Rep berasaskan OCCT menyediakan keupayaan pepejal, permukaan, topologi dan visualisasi untuk aliran CAD profesional.</p></td>
  </tr>
  <tr>
    <td width="50%" valign="top"><h3>Integriti dokumen</h3><p>Transaksi dokumen, identiti berasaskan ObjectId, rujukan kekal dan buat asal atau buat semula per arahan melindungi kesinambungan model.</p></td>
    <td width="50%" valign="top"><h3>Desktop profesional</h3><p>Arahan reben, panel boleh dok, ruang pandang MDI, pemilihan, interaksi dan meja kerja modular menyokong pertumbuhan produk jangka panjang.</p></td>
  </tr>
</table>

## Bekerjasama dengan Tusyndon

<p align="center">
  <strong>Bawakan aliran kejuruteraan sebenar, model wakil, set lukisan atau projek IFC.</strong><br />
  Kami menilai kesesuaian produk berdasarkan input, proses dan hasil penyerahan sebenar yang diperlukan.
</p>

<p align="center">
  <a href="https://www.tusyndon.com/solutions"><strong>Penyelesaian kejuruteraan</strong></a> &nbsp; · &nbsp;
  <a href="https://www.tusyndon.com/industries"><strong>Industri</strong></a> &nbsp; · &nbsp;
  <a href="mailto:sales@tusyndon.com"><strong>Tempah demo produk</strong></a> &nbsp; · &nbsp;
  <a href="https://github.com/Ludwigstrasse/SolidDesigner/discussions"><strong>Perbincangan kejuruteraan</strong></a>
</p>

---

<p align="center">
  <strong>Tusyndon Industrial Software</strong><br />
  <sub>Reka bentuk mekanikal · persediaan pembuatan · penyerahan kejuruteraan · data BIM</sub><br /><br />
  <a href="https://www.tusyndon.com/">Syarikat</a> ·
  <a href="https://www.tusyndon.com/products">Produk</a> ·
  <a href="https://github.com/Ludwigstrasse/SolidDesigner">SolidDesigner</a> ·
  <a href="https://github.com/hananiahhsu/SolidDesignerWiki">Wiki awam</a> ·
  <a href="mailto:sales@tusyndon.com">sales@tusyndon.com</a>
</p>

<details>
  <summary><strong>Aktiviti kejuruteraan GitHub</strong></summary>
  <br /><p align="center"><img src="assets/stats/metrics.svg" width="100%" alt="Metrik aktiviti kejuruteraan GitHub" /></p>
</details>
